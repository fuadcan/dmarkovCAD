...
twostate_D <- function(dname){
  dat <- get(load(paste0(dname,".rda")))
  dat <- dat/10e10
  
  lowerV <- c(-2,-2,.8,.8,0.001,-5)
  ...
  inits  <- c(1.2,  1.7,  0.9,  0.9,  0.01, 0.1)
  ...
  ress    <- lapply(1:ncol(dat), function(i) {cat(paste0(i,"\n"))
    tryCatch(constrOptim(inits, function(p) -lnviD2(p,processdat(dat[,i])), NULL, ui = const_mat[,-7], const_mat[,7]),error=function(e) list(NA,NA))})

  save(ress, file = paste0("output/",dname,"_2stateD_ress.rda"))
  
  estms <- sapply(ress,function(res) c(res$par,res$value,res$convergence))
  colnames(estms) <- colnames(dat)
  estms <- t(estms)
  colnames(estms) <- c("d_1","d_2","P_11","P_22","sigma","mu","Neg_lklihood","Convergence")
