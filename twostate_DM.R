...
twostate_DM <- function(dname){
  ...
  save(ress, file = paste0("output/",dname,"_2stateDM_ress.rda"))
  
  estms <- sapply(ress,function(res) c(res$par,res$value,res$convergence))
  colnames(estms) <- colnames(dat)
  estms <- t(estms)
  colnames(estms) <- c("d_1","d_2","P_11","P_22","sigma","mu_1","mu_2","Neg_lklihood","Convergence")
  write.csv(estms, paste0("output/",dname,"2_stateDM_ress.csv"))
